...
The game is small, mainly because my Bacione [^1] editor on mobile isn’t designed for large projects, but also because I like simplicity.
...
Previous games I made didn’t use standard game algorithms, so I wanted to try learning them. I've never used `hypot`, `sin`, `cos`, `atan`, etc. Those functions just all sounded so CPU intensive, and I’m a minimalist, right? For Marslander [^1] I simply did `y > mountain[floor(x)]` and for Ski `x - skier < tree && tree < x + skier`.
...
I’m tempted to do a diff tutorial.[^5] It is generated semi-automatically with JavaScript. Perhaps I should put the generator in a repository for others to use. I also have a generator for YouTube demos. So the toolbox of utilities seems to keep growing. Isn't that always the case?

[^1]: [Bacione Editor](https://github.com/bacionejs/editor)
...
[^5]: [Diff Tutorial Generator](https://bacionejs.github.io/editor/Games.pdf)
